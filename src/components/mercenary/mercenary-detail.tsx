...
export function MercenaryDetail({ mercenary_id }: { mercenary_id: number }) {
    ...
    const mercenaryInfo = mercenary.find((mercenary) => mercenary.id === mercenary_id);
    ...
    return (
        <Dialog>
            ...
            <DialogContent className="max-w-lg lg:max-w-[1000px] ">
                <DialogHeader>
                    <DialogTitle className="flex gap-2 items-center">{mercenaryInfo?.name}
                        <Badge variant="secondary">{mercenaryInfo.id.toString().padStart(3, '0')}</Badge>
                        <Badge className={cn("bg-gray-700 flex h-6 w-20 items-center justify-center rounded-md px-1 text-xs font-semibold text-white", {
                            'bg-blue-700 hover:bg-blue-800': mercenaryInfo.type === 'Premium',
                            'bg-orange-700 hover:bg-orange-800': mercenaryInfo.type === 'Rare',
                            'bg-purple-700 hover:bg-purple-800': mercenaryInfo.type === 'Unique',
                            'bg-green-700 hover:bg-green-800': mercenaryInfo.type === 'Reform',
                            'bg-pink-700 hover:bg-pink-800': mercenaryInfo.type === 'Idol',

                        })}>
                            {mercenaryInfo.type}
                        </Badge>
                        <Badge className="bg-gray-700 mr-2 flex h-6 w-20 items-center justify-center rounded-md px-1 text-xs font-semibold text-white">{mercenaryInfo.AttackType}</Badge>
                    </DialogTitle>
                ...
                <div className="grid lg:grid-cols-2 gap-4">
                    <div className="flex justify-center items-center">
                        <Tabs defaultValue="images" className="w-full">
                            ...
                            <TabsContent value="images" className="flex justify-center items-center w-full">
                                {/* Male Image */}
                                <motion.div
                                    style={{
